...
import { protect, authorize, requireMentorApproval } from "../middleware/auth.js";
...
// routes/communities.js (UPDATED - Add to your existing file)
// Only showing the CREATE route that needs updating

// Create community (approved mentors only) - UPDATED WITH CREDIT DEDUCTION
router.post("/", protect, authorize("mentor"), requireMentorApproval, async (req, res) => {
  try {
    const { name, description, category, tags, joinCost, maxMembers, coverImage, settings } = req.body;
...
// Create community (approved mentors only)
router.post("/", protect, authorize("mentor"), requireMentorApproval, async (req, res) => {
  try {
    const { name, description, category, tags, joinCost, maxMembers, coverImage, settings } = req.body;
...
router.put("/:id", protect, authorize("mentor"), requireMentorApproval, async (req, res) => {
  try {
    const community = await Community.findById(req.params.id);
    ...
    const updates = {};
    const allowedFields = ["name", "description", "category", "tags", "joinCost", "maxMembers", "coverImage", "settings", "isActive"];
...
router.get("/mentor/my-communities", protect, authorize("mentor"), async (req, res) => {
  try {
...
router.get("/:id/pending-requests", protect, authorize("mentor"), async (req, res) => {
  try {
    const community = await Community.findById(req.params.id);
...
router.put("/:communityId/requests/:requestId", protect, authorize("mentor"), async (req, res) => {
  try {
    ...
    if (action === "approve") {
      ...
      if (community.maxMembers && community.statistics.totalMembers >= community.maxMembers) {
      ...
      await Community.findByIdAndUpdate(req.params.communityId, {
        $inc: { "statistics.totalMembers": 1, "statistics.pendingRequests": -1 },
      });
      ...
      res.json({ success: true, message: "Request rejected and credits refunded" });
    }
  } catch (error) {
...
router.post("/:id/join", protect, authorize("student"), async (req, res) => {
  try {
    ...
    if (!community.isActive) {
      return res.status(400).json({ success: false, error: "Community is not active" });
    }
    ...
    if (existingMembership) {
      return res.status(400).json({
        success: false,
        error: existingMembership.status === "approved" 
          ? "Already a member" 
    ...
    if (community.settings.autoApprove) {
      await Community.findByIdAndUpdate(req.params.id, {
        $inc: { "statistics.totalMembers": 1, "statistics.totalRevenue": community.joinCost },
      });
      await User.findByIdAndUpdate(req.user._id, {
        $inc: { "statistics.communitiesJoined": 1 },
      });
      ...
      await Community.findByIdAndUpdate(req.params.id, {
        $inc: { "statistics.pendingRequests": 1, "statistics.totalRevenue": community.joinCost },
      });
    }
...
router.get("/student/my-communities", protect, authorize("student"), async (req, res) => {
  try {
